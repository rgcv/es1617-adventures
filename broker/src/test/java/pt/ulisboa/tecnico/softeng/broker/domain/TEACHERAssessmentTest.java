...
//FIXME		assertEquals(Operation.Type.WITHDRAW.toString(), bankOperationData.getType());
...
//FIXME		assertEquals(Room.Type.SINGLE.toString(), roomBookingData.getRoomType().toString());
		...
		adventure.setPaymentConfirmation("FAKE CONFIRMATION");
		for (int i = 0; i < ConfirmedState.MAX_ERRORS - 1; i++) {
			adventure.process();
		}
...
//FIXME		adventure.process();
//FIXME		assertEquals(State.CANCELLED, adventure.getState());
//FIXME		assertNotNull(adventure.getPaymentCancellation());
//FIXME		assertNotNull(adventure.getActivityCancellation());
//FIXME		assertNotNull(adventure.getRoomCancellation());
		// money is reimbursed after cancellation
//FIXME		assertEquals(balance, this.account.getBalance());
		// availability is increased
//FIXME		assertEquals(offerAvailability, this.offerArrivalDeparture.getNumberOfBookings());

		// test getOperationData for cancellation
//FIXME		bankOperationData = BankInterface.getOperationData(adventure.getPaymentCancellation());
//FIXME		assertEquals(Operation.Type.DEPOSIT.toString(), bankOperationData.getType());
//FIXME		assertEquals(AMOUNT, bankOperationData.getValue());

		// test getRoomBookingData for cancellation
//FIXME		roomBookingData = HotelInterface.getRoomBookingData(adventure.getRoomCancellation());
//FIXME		assertEquals(Room.Type.SINGLE.toString(), roomBookingData.getRoomType().toString());
//FIXME		assertNotNull(roomBookingData.getCancellation());

		// test getActivityReservationData for cancellation
//FIXME		activityReservationData = ActivityProvider.getActivityReservationData(adventure.getActivityCancellation());
//FIXME		assertEquals(arrival, activityReservationData.getBegin());
//FIXME		assertNotNull(activityReservationData.getCancellation());
//FIXME		assertNotNull(activityReservationData.getCancellationDate());
...
//FIXME		assertEquals(this.roomCounter - 2, bulkRoomBooking.getReferences().size());
//FIXME		assertEquals(Room.Type.DOUBLE.toString(), Hotel.getRoomBookingData(reference).getRoomType());
